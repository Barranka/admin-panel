    ...
    <div class="modal__wrapper" @click.self="$emit('close')">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <el-form :model="user" ref="user" :rules="rules" label-width="120px" class="form">

            <el-form-item prop="name" label="Имя" :rules="rules.name">
              <el-input placeholder="Введите ваше имя" v-model="user.name"></el-input>
            </el-form-item>

            <el-form-item prop="firstname" label="Фамилия" :rules="rules.firstname">
              <el-input placeholder="Введите вашу фамилию" v-model="user.firstname"></el-input>
            </el-form-item>

            <el-form-item prop="phone" label="Телефон" :rules="rules.phone">
              <el-input placeholder="Введите номер телефона" v-model="user.phone"></el-input>
            </el-form-item>

          </el-form>
          <el-row class="form__button">
            <el-button type="primary" @click="submitForm('user')">Сохранить</el-button>
            <el-button type="primary" @click="[$emit('close'), resetForm('user')]">Отменить</el-button>
          </el-row>
        </div>
      </div>
    </div>
    ...
    return {
      rules: {
        ...
        phone: [
          ...
          { max: 11, message: "Введите корректный номер из 11 цифр", trigger: "change" }
        ]
      }
    }
  },
  ...
  methods: {
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) this.$emit('save', this.user)
      })
    },
    resetForm(formName) {
      this.user.name = ''
